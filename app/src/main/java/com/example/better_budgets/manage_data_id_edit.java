...
public class manage_data_id_edit extends AppCompatActivity {
    Integer spendingid;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_manage_data_id_edit);

        EditText editId = findViewById(R.id.editText_md_id_edit_id);
        EditText editDate = findViewById(R.id.editText_md_id_edit_date);
        EditText editSource = findViewById(R.id.editText_md_id_edit_source);
        EditText editAmount = findViewById(R.id.editText_md_id_edit_amount);
        EditText editSeller = findViewById(R.id.editText_md_id_edit_seller);

        Intent intent = getIntent();
        spendingid = intent.getIntExtra("spendingid", -1);

        if (spendingid != -1) {
            Spending spending = manage_data_id.spendings_id.get(spendingid);
